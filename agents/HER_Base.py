...
class HER_Base:
    ...
    def save_alternative_experience(self):
        ...
        new_states = [self.create_state_from_observation_and_desired_goal(observation, new_goal) for observation in
                      self.episode_observations]
        ...
        self.HER_memory.add_experience(new_states, self.episode_actions, new_rewards, new_next_states,
                                       self.episode_dones)
    ...
    def sample_from_HER_and_Ordinary_Buffer(self):
        ...
        states = ops.cat((states, HER_states))
        actions = ops.cat((actions, HER_actions))
        rewards = ops.cat((rewards, HER_rewards))
        next_states = ops.cat((next_states, HER_next_states))
        dones = ops.cat((dones, HER_dones))
