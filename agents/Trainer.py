...
class Trainer:
    ...
    def run_games_for_agents(self):
        ...
        for _, agent_class in enumerate(self.agents):
            ...
            self.run_games_for_agent(agent_class)
    ...
    def create_object_to_store_results(self):
        ...
        if self.config.overwrite_existing_results_file or \
                not self.config.file_to_save_data_results or not os.path.isfile(self.config.file_to_save_data_results):
            results = {}
        else:
    ...
    def run_games_for_agent(self, agent_class):
        ...
        agent_round = 1
        for _ in range(self.config.runs_per_agent):
            ...
            print("f\033[1m" + "{agent_number}.{agent_round}: {agent_name}" + "\033[0m", flush=True)
    ...
    def visualise_overall_agent_results(self, agent_results, agent_name, show_mean_and_std_range=False,
        ...
        if show_mean_and_std_range:
            ...
            for ix, result in enumerate(agent_results):
                ...
                plt.plot(x_vals, result, label=agent_name + f"_{(ix + 1)}", color=color)
    ...
    def ignore_points_after_game_solved(self, mean_minus_x_std, mean_results, mean_plus_x_std):
        ...
        ix = None
        for ix, mean_results_ele in enumerate(mean_results):
            if mean_results_ele >= self.config.environment.get_score_to_win():
                break
    ...
    def visualise_set_of_preexisting_results(
            self, results_data_paths, save_image_path=None, show_image=True, plot_titles=None, y_limits=(None, None)
            # y_limits = [None, None]
    ):
        ...
        for ax_ix, results_data_paths_element in enumerate(results_data_paths):
            self.visualise_preexisting_results(show_image=False, data_path=results_data_paths_element, ax=axes[ax_ix],
