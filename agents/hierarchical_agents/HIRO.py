...
class HIRO(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.higher_level_agent = HIRO_Higher_Level_DDPG_Agent(self.higher_level_agent_config,
                                                               self.lower_level_agent.actor_local)

        self.step_lower_level_states = []
        self.step_lower_level_action_seen = []
        self.higher_level_agent.step_lower_level_states = None
        self.higher_level_agent.step_lower_level_action_seen = None
...
class HIRO_Higher_Level_DDPG_Agent(DDPG):
    ...
    def save_experience(self, memory=None, experience=None):
        ...
        if experience is None:
            experience = (
                self.step_lower_level_states, self.step_lower_level_action_seen, self.reward,
    ...
    def sample_experiences(self):
        ...
        states = ms.Tensor(np.vstack(states), dtype=ms.float32)
        actions = ms.Tensor(np.vstack(actions), dtype=ms.float32)
        rewards = ms.Tensor(np.vstack(rewards), dtype=ms.float32)
        next_states = ms.Tensor(np.vstack(next_states), dtype=ms.float32)
        dones = ms.Tensor(np.vstack([int(done) for done in dones]), dtype=ms.float32)
    ...
    def transform_goal_to_one_most_likely_to_have_induced_actions(self, experience):
        ...
        goal_candidate_state_change_random_iterations = [np.random.normal(goal_candidate_state_change[0]) for _ in
                                                         range(self.number_goal_candidates - 2)]
        ...
        for goal_ix, goal in enumerate(goal_candidates):
            ...
            if log_probability_total >= _max:
                _max = log_probability_total
    ...
    def log_probability_lower_level_picks_action(self, state, goal, action):
        ...
        and goal as inputs"""
        state_and_goal = ms.Tensor(np.concatenate((state, goal)), dtype=ms.float32).unsqueeze(0)
        ...
        return -0.5 * ms.ops.norm(action - action_would_have_taken, 2) ** 2
...
class Lower_Level_Agent_Environment_Wrapper(Wrapper):
    ...
    def __init__(self, env, HIRO_agent, max_sub_policy_timesteps):
        Wrapper.__init__(self, env)
        self.lower_level_timesteps = None
        self.env = env
    ...
    def turn_internal_state_to_external_state(self, internal_state, goal):
        """turn_internal_state_to_external_state"""
