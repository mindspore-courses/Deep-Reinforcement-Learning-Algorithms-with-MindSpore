...
class SNN_HRL(Base_Agent):
    """Implements the hierarchical RL agent that uses stochastic neural networks (SNN) from the paper Florensa et al.
    2017 https://arxiv.org/pdf/1704.03012.pdf Works by: 1) Creating a pre-training environment within which the
    skill_agent can learn for some period of time 2) Then skill_agent is frozen 3) Then we train a manager agent that
    chooses which of the pre-trained skills to let act for it for some period of time Note that it only works with
    discrete states at the moment.

    Note that this agent will not work well in environments where it is beneficial to end the game as quickly as
    possible because then there isn't enough incentive for the skills to learn to explore different parts of the
    state space
    """
    agent_name = "SNN-HRL"
    ...
    def __init__(self, config):
        ...
        assert isinstance(self.environment.reset(), (int, np.int64)) or \
               self.environment.reset().dtype == np.int64, "only works for discrete states currently"
    ...
    def create_skill_training_agent(self):
        ...
        self.skill_agent_config.environment = Skill_Wrapper(
            copy.deepcopy(self.environment), self.environment.observation_space.n,
            self.num_skills, self.skill_agent_config.hyperparameters["regularisation_weight"],
            self.skill_agent_config.hyperparameters["visitations_decay"]
        )
...
class Skill_Wrapper(Wrapper):
    ...
    def observation(self, observation):
        """get observation"""
        return np.concatenate((np.array(observation).reshape(-1), np.array([self.skill])))
    ...
    def update_state_visitations(self, next_state):
        ...
        if len(next_state.shape) < 1:
            self.state_visitations[self.skill][next_state.item()] += 1
        else:
    ...
    def calculate_probability_correct_skill(self, next_state):
        ...
        if len(next_state.shape) < 1:
            visitations_correct_skill = self.state_visitations[self.skill][next_state.item()]
            visitations_any_skill = np.sum([visit[next_state.item()] for visit in self.state_visitations])
        else:
    ...
    def print_state_distribution(self):
        ...
        for state_visitations_element in self.state_visitations:
            for state in range(len(self.state_visitations[0])):
                state_count[state] += state_visitations_element[state]
...
class Manager_Frozen_Worker_Wrapper(Wrapper):
    ...
    def step(self, action):
        ...
        for _ in range(self.timesteps_before_changing_skill):
            skill_action = self.skills_agent.pick_action(np.array([next_state.item(), action]))
