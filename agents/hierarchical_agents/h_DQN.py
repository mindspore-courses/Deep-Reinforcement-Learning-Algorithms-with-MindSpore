...
class h_DQN(Base_Agent):
    """Implements hierarchical RL agent h-DQN from paper Kulkarni et al. (2016)
    https://arxiv.org/abs/1604.06057?context=stat Note also that this algorithm only works when we have discrete
    states and discrete actions currently because otherwise it is not clear what it means to achieve a subgoal state
    designated by the meta-controller"""
    agent_name = "h-DQN"

    def __init__(self, config):
        Base_Agent.__init__(self, config)
        self.meta_controller_next_state = None
        self.sub_goal = None
        self.meta_controller_state = None
        self.meta_controller_steps = None
        self.sub_goal_achieved = None
        self.episode_over = None
        self.cumulative_meta_controller_reward = None
        ...
        self.controller.q_network_local = self.create_NN(
            input_dim=self.state_size * 2, output_dim=self.action_size, key_to_use="CONTROLLER"
        )
    ...
    def reset_game(self):
        ...
        self.update_learning_rate(self.controller_config.hyperparameters["learning_rate"],
                                  self.controller.q_network_optimizer)
        self.update_learning_rate(self.meta_controller_config.hyperparameters["learning_rate"],
    ...
    def step(self):
        ...
        while not self.episode_over:
            ...
            while not (self.episode_over or self.sub_goal_achieved):
                self.pick_and_conduct_controller_action()
                self.update_data()
                if self.time_to_learn(self.controller.memory, self.global_step_number,
            ...
            self.save_experience(memory=self.meta_controller.memory,
                                 experience=(
                                     self.meta_controller_state, self.sub_goal, self.cumulative_meta_controller_reward,
        ...
        if self.episode_number % 100 == 0:
            print(" ")
            print(
                "Most common goal -- {} -- ".format(max(set(self.goals_seen[-100:]), key=self.goals_seen[-100:].count))
            )
    ...
    def update_meta_controller_data(self):
        ...
        if self.done:
            ...
            self.meta_controller_next_state = self.environment.s
