...
class Base_Agent:
    ...
    def take_optimisation_step(self, optimizer, grads, clipping_norm=None):
        ...
        if clipping_norm is not None:
            grads = ops.clip_by_global_norm(grads, clipping_norm)
        optimizer(grads)
    ...
    def soft_update_of_target_network(self, local_model, target_model, tau):
        ...
        so the target network's parameter values trail the local networks. This helps stabilise training"""
    ...
    def create_NN(self, input_dim, output_dim, key_to_use=None, override_seed=None, hyperparameters=None):
        ...
        return Network(
            ...
            layers_info=hyperparameters["linear_hidden_units"],
...
class Network(nn.Cell):
    ...
    def __init__(self, input_dim: int, output_dim: int, layers_info: list, hyperparameters):
        super().__init__()
        activation = nn.ReLU()
        if hyperparameters["hidden_activations"] == 'relu':
            activation = nn.ReLU()

        batch_norm = hyperparameters["batch_norm"]
        self.y_range = hyperparameters["y_range"]

        if len(layers_info) == 0:
            nets = nn.Dense(input_dim, output_dim)
        else:
            start = nn.Dense(input_dim, layers_info[0])
            end = nn.Dense(layers_info[-1], output_dim)
            if batch_norm:
                nets = [start, activation, nn.BatchNorm1d(layers_info[0]), nn.Dropout(p=hyperparameters["dropout"])]
            else:
                nets = [start, activation, nn.Dropout(p=hyperparameters["dropout"])]

            for i in range(len(layers_info) - 1):
                nets.append(
                    nn.Dense(layers_info[i], layers_info[i + 1])
                )
                nets.append(activation)
                if batch_norm:
                    nets.append(nn.BatchNorm1d(num_features=layers_info[i + 1]))
                nets.append(nn.Dropout(p=hyperparameters["dropout"]))
            nets.append(end)
