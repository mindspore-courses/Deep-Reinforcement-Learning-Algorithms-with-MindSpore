...
class Prioritised_Replay_Buffer(Max_Heap, Deque):
    ...
    def separate_out_data_types(self, experiences):
        ...
        for e in experiences:
            states_append(e.value[self.indexes_in_node_value_tuple["state"]])
            actions_append(e.value[self.indexes_in_node_value_tuple["action"]])
            rewards_append(e.value[self.indexes_in_node_value_tuple["reward"]])
            next_states_append(e.value[self.indexes_in_node_value_tuple["next_state"]])
            dones_append(int(e.value[self.indexes_in_node_value_tuple["done"]]))

        states = ms.Tensor(states, ms.float32)
        actions = ms.Tensor(actions, ms.float32).unsqueeze(-1)
        rewards = ms.Tensor(rewards, ms.float32).unsqueeze(-1)
        next_states = ms.Tensor(next_states, ms.float32)
        dones = ms.Tensor(dones, ms.float32).unsqueeze(-1)
